...
```shell
Open the py files using IDLE (right click and Edit with IDLE)
For Mac users: Open the `package_installer_mac.py` and run the file
For Windows users: Open the `package_installer_windows.py` and run the file
```
